...
int get_repeat_count(int n) {
    ...
    else if (n < 100000) return 100;
    else return 20;
}

int main() {
    int n_list[] = {256, 1024, 4096, 16384}; 
    int n_count = sizeof(n_list) / sizeof(int);

    cout << "问题规模n,重复次数,平凡算法时间(秒),优化算法时间(秒),性能提升比" << endl;
    ...
    for (int i = 0; i < n_count; i++) {
        ...
        cout << n << "," << repeat << "," 
             ...
             << fixed << setprecision(4) << speedup << endl;

        free_matrix(mat, n);
        free_vector(vec);
    }

    cout << "\n✅ 平凡算法 vs Cache优化算法 性能对比完成！数据已输出为CSV格式。" << endl;
    return 0;
}