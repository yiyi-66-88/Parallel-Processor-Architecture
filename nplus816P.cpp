...
void compare_precision(float res_naive, float res_opt, const string& opt_name) {
    ...
    float rel_error = abs_error / (fabs(res_naive) + 1e-9); // Avoid division by zero
    cout << "  " << opt_name << " absolute error: " << abs_error << endl;
    cout << "  " << opt_name << " relative error: " << rel_error << endl;
}
...
int main() {
    ...
    cout << "n=" << n << " naive algorithm time: " << t_naive << " s" << endl;
    cout << "n=" << n << " 8-way unrolling time: " << t_eight << " s" << endl;
