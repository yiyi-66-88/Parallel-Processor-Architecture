...
float mat_vec_dot_opt_block(float** mat, float* vec, int n, int block_size=64) {
    if (mat == nullptr || vec == nullptr || n <= 0) return 0.0f;
...
int main() {
    ...
    for (int i = 0; i < n_count; i++) {
        ...
        double t_opt_block = time_mat_vec(mat, vec, n, repeat, 
            [=](float** m, float* v, int sz) { return mat_vec_dot_opt_block(m, v, sz, BLOCK_SIZE); });

        double speedup_opt = t_naive / t_opt;
        double speedup_block = t_naive / t_opt_block;
